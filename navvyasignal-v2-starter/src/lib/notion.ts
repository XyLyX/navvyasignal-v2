...
export type Story = {
 ...
 today: boolean; watchlist: boolean; watchStatus: string; nextReview: string | null;
...
type Prop = { title?: Text[]; rich_text?: Text[]; select?: {name:string}|null;
 checkbox?: boolean; date?: {start:string}|null; multi_select?: {name:string}[] };
...
export async function getStories(limit = 800):Promise<Story[]> {
 ...
 // Read all eligible pages before selecting homepage stories; Notion query order is not editorial rank.
 do {
  ...
  for(const page of data.results){
   ...
   // Editorial approval is required before showing records publicly.
