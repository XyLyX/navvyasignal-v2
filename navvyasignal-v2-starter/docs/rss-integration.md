...
- **Image**: the feed item's own article-level `media:content` (image), `media:thumbnail` or image `enclosure` comes first (https, allowlisted host). Channel-level `<image>` and site logos are never used. If a displayed network item has no feed image, live builds look at the original article page (`src/lib/rss/ogImage.ts`; displayed items only, 5 s timeout, 512 KB cap, allowlisted-host redirects only) and accept its `og:image` only if it is https on an allowlisted host, is not the homepage's OG image, is not shared by two articles, does not look like a logo, icon or default (`og-image`, `logo`, `favicon`, …) and answers a `HEAD` request as an image under 5 MB. Otherwise the card keeps its branded text tile. The result is tagged `imageSource: 'feed' | 'og'`. OG lookups are skipped in fixture mode, so CI stays offline.
- **Dedupe and order**: unique by canonical URL (host without `www`, no trailing slash, tracking params removed) across all six feeds, earlier allowlist entries winning. Newest first by the real instant (time zones honoured). Items with an identical timestamp keep the publisher's own feed order (`feedIndex`), then canonical URL, so the result is deterministic and matches the order on the publication's own index page.
...
- **Hero**: compact (about 323 px desktop, 263 px at 375 px wide); the eyebrow reads "GLOBAL INTELLIGENCE" (PREVIEW removed); headline, copy and the seven-desk line are unchanged.
- **A Different Lens**: the text link is replaced by a square Explore tile (`.nl-tile`) that shows Navyaa's own logo, `https://navyaa.blog/images/logo.png`, and links to https://navyaa.blog/. That logo is the one Navyaa declares in its own structured data (`"logo"` in the page JSON-LD); its declared favicon and touch icon return 404 and its header logo is plain text, so this PNG is the only genuine logo asset. It is hot-linked from an allowlisted host (not copied into this repository) with a text "Navyaa" tile underneath as a fallback. On desktop the tile sits in the third grid column, directly above the third essay; on tablet and mobile it stacks under the heading. The three essays still come from the RSS feed.
- **From the Navvya Network**: the homepage now uses the same `NetworkPanels` component as `/network` (six equal panels, `heading` prop only differs), replacing the earlier homepage-only layout. The homepage venture cards ("The Navvya Network") and the `/network` directory are unchanged.
- **Covers**: an article with no authentic feed or article-level OG image gets a CSS-only branded editorial cover (per-publication palette, monogram and content-type rule). No stock photos, channel logos or site-default OG images are ever used; a real image always stacks above the cover.

### Join Our Network (`/network/join`): preview only

The repository has no form infrastructure: `/contact` is a `mailto:hello@navvyasignal.com` link, there is no API route (static export), and `netlify.toml` has no forms configuration. The join page therefore validates in the browser and never transmits anything; it says so and points to the existing contact address. Making it live needs an explicit decision on one of: (a) Netlify Forms on the isolated V2 site (`data-netlify` markup, a Netlify-side notification destination, spam protection); (b) an approved form/CRM endpoint or serverless function; or (c) keeping `mailto:`. No endpoint or email automation was created.

## Zen Homes feed audit (24 Sep 2026)

- **Insights**: the feed, the published `/insights/` index and `sitemap.xml` all list the same 17 articles; none is missing. The newest is dated 28 Aug 2026.
- **Portfolio**: the feed lists 9 items newest first and matches the `/portfolio` page (`data-published` 22 Sep, 22 Sep, 19 Sep, 3 Sep, 31 Aug x4, 28 Aug). The apparent misordering came from this importer breaking same-timestamp ties alphabetically (Binghatti Starfall before Maldives Waterfront), plus the 2-item cap hiding the 19 Sep item. Ties now keep the publisher's order.
